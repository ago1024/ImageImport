...
namespace PreviewHandlers {
	...
	public class PreviewHandlerHost : Control {
		...
		private Guid GetGuid(string filename, string interf)
		{
			string guid = AssocQueryString(AssocStr.ShellExtension, Path.GetExtension(filename), interf);
			if (guid != null) return new Guid(guid);
		...
		private Guid GetPreviewHandlerGUID(string filename) {
			...
			string guid = AssocQueryString(AssocStr.ShellExtension, Path.GetExtension(filename), "{8895b1c6-b41f-4c1c-a562-0d564250836f}");
			if (guid != null) return new Guid(guid);
			...
			if (ext != null) {
				...
				string[] classNames = { Convert.ToString(ext.GetValue(null)) };
				IEnumerable<string> classes = classNames;

				string perceivedType = Convert.ToString(ext.GetValue("PerceivedType"));
				if (perceivedType != null) classes = classes.Concat(new string[] { @"SystemFileAssociations\" + perceivedType });

				RegistryKey openWithProgids = ext.OpenSubKey("OpenWithProgids");
				if (openWithProgids != null) classes = classes.Concat(openWithProgids.GetValueNames());
		...
		static string AssocQueryString(AssocStr association, string extension, string guid)
		{
			uint length = 512;
			var sb = new StringBuilder((int) length); // (length-1) will probably work too as null termination is added
			uint ret = AssocQueryString(AssocF.None, association, extension, guid, sb, ref length);
